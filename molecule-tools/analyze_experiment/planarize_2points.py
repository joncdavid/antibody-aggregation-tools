...
def are_colinear(v,w):
    """v and w are vectors of the same dimension. v and w are co-linear
    and only if their unit vectors are the same."""
    v_unit = get_unit_vector(v)
    w_unit = get_unit_vector(w)
    
    f = (lambda x,y: x and y)
    is_colinear = reduce( f, (v_unit == w_unit))
    is_colinear_byNegative = reduce( f, (-1*v_unit == w_unit))
    
    return is_colinear or is_colinear_byNegative

def is_onPlane(v,p):
    """A vector v exists on a particular plane iff
    the dot product of v and p (the normal vector of plane)
    is zero."""
    
    ## why? by definition, dot product of v and p is |a| * |b| * cos(theta)
    ## if they are orthogonal, then theta = 90. Therefore, cos(90) = 0.
    ## 0 times anything is 0.
    is_coplanar = (np.dot(v,p) == 0)
    return is_coplanar

def are_coplanar(a, b, c):
    """Three points a,b,c are co-planar if they exist on the same plane.
    These points exist on the same plane iff 
    the vectors (ab x bc), (bc, ca), and (ca x ab) are co-linear."""
    
    ab = np.array( a-b )
    bc = np.array( b-c )
    ca = np.array( c-a )
    
    ab_cross_bc = np.cross(ab,bc)
    bc_cross_ca = np.cross(bc,ca)
    ca_cross_ab = np.cross(ca,ab)
    
    result1 = are_colinear(ab_cross_bc, bc_cross_ca)
    result2 = are_colinear(bc_cross_ca, ca_cross_ab)
    result3 = are_colinear(ca_cross_ab, ab_cross_bc)
...
def solve(p1,p2):
    ...
    #phi_x = phi_x if (p2[1] < 0) else -phi_x  ## this isn't always true... 
    phi_x = -phi_x  ## change my signs if p2rot's y value isn't 0.; delete me after solving myo1n1cV5
...
def ut_are_colinear():
    ...
    result1 = are_colinear(u,v)
    result2 = are_colinear(v,w)
    
    print( "==== ut_are_colinear ====" )
    print( "u: {}".format(u))
    print( "v: {}".format(v))
    print( "w: {}".format(w))
    print( "u and v are co-linear: {}".format( result1 ))
    print( "v and w are co-linear: {}".format( result2 ))
    

def ut_is_onPlane():
    v = np.array( [1,0,0] )
    p = np.array( [0,1, 0] )
    result = is_onPlane(v, p)
    
    print( "==== ut_is_onPlane ====")
    print( "v: {}".format(v) )
    print( "p: {}".format(p) )
    print( "are co-planar: {}".format(result))

def ut_are_coplanar():
    p1 = np.array( [0,0,0] )
    p2 = np.array( [1,0,0] )
    p3 = np.array( [0,0,1] )
    result = are_coplanar(p1,p2,p3)
    
    print( "==== ut_are_coplanar ====" )
    print( "p1: {}".format(p1) )
    print( "p2: {}".format(p2) )
    print( "p3: {}".format(p3) )
    print( "p1,p2,p3 are co-planar: {}".format( result ))
...
def ut_solve():
    ...
    # mb4n
    #ut_solve_testX( np.array( [3.929, 9.897, -0.472] ),
    #                np.array( [-0.576, 17.061, -9.827] ) )
    
    # myo1n1cV2 "open"
    #ut_solve_testX( np.array( [-13.408, -16.50, -2.789] ),
    #                np.array( [-4.648, 1.214, 10.026] ) )
    
    # myo1n1cV4 "closed"
    #ut_solve_testX( np.array( [ -15.443,  -8.964,  11.321] ),
    #                np.array( [ -11.172,   7.458,   0.682] ) )
    
    # myo2nV5
    #ut_solve_testX( np.array( [  -4.181, -13.095, -10.941] ),
    #                np.array( [  13.030,   0.441, -13.072] ) )
    
    # myo1n1cV4
    #ut_solve_testX( np.array( [ -16.800,  -8.569,  10.442] ),
    #                np.array( [ -11.498,   4.885,  -4.990] ) )
    
    # myo1n1cV5
    #ut_solve_testX( np.array( [9.710, -18.677, 3.444] ),
    #                np.array( [3.707, -0.149, 11.708] ) )
    
    # myo2nV5
    #ut_solve_testX( np.array( [-3.921, -13.269, -10.738] ),
    #                np.array( [11.428, 0.418, -13.376] ) )
    
    ## myo1n1av2
    #ut_solve_testX( np.array( [7.379, -16.615, -10.038] ),
    #                np.array( [-20.219, 10.992, 3.576] ) )

    ## myo1nv4
    ut_solve_testX( np.array( [16.554, -2.771, -8.766] ),
                    np.array( [0,0,0] ) )
