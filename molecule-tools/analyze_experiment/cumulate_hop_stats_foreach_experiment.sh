#!/usr/bin/env bash
# Filename: cumulate_hop_stats_foreach_experiment.sh
# Author: Jon David
# Date: Thursday, January 2, 2020
# Description:
#   For each experiment found in some directory,
#   cumulate the hopkins statistics into a single matrix.
#-----------------------------------------------------------------
...
function cumulate_hopstats_for_single_experiment {
    #echo "We are now in function cumulate_hopstats_for_single_experiment"
    experimentDir=$1
    experimentName=$2
    #maxAggregateSize=$3
    ...
    (cd $experimentDir;
     $ANALYSIS_TOOLS_DIR/cumulate_hop_stats.sh $experimentName)
    ### $ANALYSIS_TOOLS_DIR/cumulate_hop_stats.sh $experimentName $maxAggregateSize)
